...
const mailTransport = nodemailer.createTransport(config as any);

const emailMap: { [key: string]: any } = {
  [UserAuthTypeEnum.register]: {
    subject: '注册 破壳AI知识库 账号',
    html: (code: string) => `<div>您正在注册 破壳AI知识库 账号，验证码为：${code}</div>`
  },
  [UserAuthTypeEnum.findPassword]: {
    subject: '修改 破壳AI知识库 密码',
    html: (code: string) => `<div>您正在修改 破壳AI知识库 账号密码，验证码为：${code}</div>`
  }
};
